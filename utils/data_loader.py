...
from utils.data_desc import AVAILABEL_DATASETS, TS_LENGTHS


def load_dataset(data_name, task, normalization=None, feature_first=False):
    def _preprocess(X, y=None):
        ...
        for i in range(len(X)):
            ...
            X_ = X.iloc[i, :].copy(deep=True)

            # 1. find the maximum length of each dimension
            all_len = [len(x) for x in X_]
            max_len = max(all_len)
    ...
    if data_name in AVAILABEL_DATASETS:
        if task == "classification":
            X_train, y_train, class_names = _preprocess(
                *load_from_tsfile_to_dataframe(
                    f"datasets/{data_name}/{data_name}_TRAIN.ts"
                )
            )
            X_test, y_test, class_names = _preprocess(
                *load_from_tsfile_to_dataframe(
                    f"datasets/{data_name}/{data_name}_TEST.ts"
                )
            )
            return X_train, y_train, X_test, y_test, class_names
        else:
            X_train, y_train = _preprocess(
                *load_from_regression_tsfile_to_dataframe(
                    f"datasets/{data_name}/{data_name}_TRAIN.ts"
                )
            )
...
def load_from_regression_tsfile_to_dataframe(full_file_path_and_name, return_separate_X_and_y=True, replace_missing_vals_with='NaN'):
